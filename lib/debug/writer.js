...
export class RokuDebugWriter {
   ...
   * @param {string|import('node:net').AddressInfo} host - Hostname of device or `AddressInfo` object
  ...
  async #lockLogfile() {
    ...
    if (_.isNil(this.#opts.logFd)) {
      ...
      log.debug(`PID ${process.pid} locked ${this.logPath}`);
      return async () => {
        try {
          await originalUnlock();
          log.debug(`PID ${process.pid} unlocked ${this.logPath}`);
        } catch (err) {
          // todo handle better
          log.warn(err);
        }
