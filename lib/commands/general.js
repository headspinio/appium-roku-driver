...
async function makePluginReq ({
  ...
  if (!_.isUndefined(encoding)) {
    reqOpts.headers['Content-Encoding'] = encoding;
  }
  log.info(`Calling ${method} ${url} ${JSON.stringify(reqOpts)}`);
...
export async function getScreenshot () {
  ...
  try {
    const img = (await makePluginReq({opts: this.opts, method: 'GET', uri, contentType: null, encoding: null})).data;
