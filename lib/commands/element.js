...
export async function _elementInteractionGuard (elId) {
  ...
  log.debug(`Examining ${testNodes.length} nodes retrievable with selector '${selector}' ` +
            `and mult flag set to ${mult}`);
...
function nodeIsFocused (n, strict = false) {
  log.debug(`Checking whether <${n.nodeName}> is focused`);
  if (!n.attributes) {
    return false;
  }

  // if the node says it's focused, it's focused
  for (const attr of Object.values(n.attributes)) {
    if (attr.name === 'focused' && attr.value === 'true') {
      log.debug(`<${n.nodeName}> is focused due to attributes`);
      return true;
    }
  }

  if (strict) {
    log.debug(`<${n.nodeName}> is not focused in strict mode`);
    return false;
  }

  log.debug(`Checking whether <${n.nodeName}> might be focused in non-strict mode`);

  // otherwise, if the parent is focused, no siblings are focused, and at least one child is
  // focused, then we should say it's focused
  const parent = n.parentNode;
  if (!parent) {
    log.debug(`<${n.nodeName}> not focused--no parent`);
    return false;
  }

  log.debug(`Checking whether parent of <${n.nodeName}> is focused`);
  if (!nodeIsFocused(parent, true)) {
    log.debug(`<${n.nodeName}> not focused--parent not focused`);
    return false;
  }

  log.debug(`Checking whether siblings of <${n.nodeName}> are focused`);
  for (let i = 0; i < _.size(parent.childNodes); i++) {
    const sibling = parent.childNodes[i];
    if (sibling.nodeType !== 1) {
      continue; // not an element
    }
    if (sibling === n) {
      continue; // avoid infinite loop, don't recheck self
    }
    if (nodeIsFocused(sibling, true)) {
      log.debug(`<${n.nodeName}> not focused--sibling was focused`);
      return false;
    }
  }

  log.debug(`Checking whether children of <${n.nodeName}> are focused`);
  let hasOneFocusedChild = false;
  for (let i = 0; i < _.size(n.childNodes); i++) {
    const child = n.childNodes[i];
    if (child.nodeType !== 1) {
      continue; // not an element
    }
    if (!nodeIsFocused(child, true)) {
      hasOneFocusedChild = true;
      break;
    }
  }

  if (!hasOneFocusedChild) {
    log.debug(`<${n.nodeName}> not focused--had no focused child`);
    return false;
  }

  log.debug(`<${n.nodeName}> is deemed focused due to parent/sibling/child focus situation`);
  return true;
}
...
export async function performActionWithKeyboard (action, elId) {
  ...
  let okBtn = null;
  try {
    okBtn = await this.findElOrEls('xpath', '//Label[@text="OK"]/ancestor::StdDlgButton', false);
  } catch (ign) {}

  if (okBtn) {
    await this.click(okBtn[W3C_WEB_ELEMENT_IDENTIFIER]);
  } else {
    log.debug('Could not find OK button, will press Back instead');
    await this.roku_pressKey({key: 'Back'});
  }

  log.debug('Waiting for keyboard to disappear');
  const numTries = 5;
  const interval = 750;
  await retryInterval(numTries, interval, async () => {
    try {
      await this.findElOrEls('xpath', kbSelector, false);
      throw new Error('Keyboard never disappeared');
    } catch (ign) {}
  });
}
...
export async function setValueByKeyboard (chars) {
  ...
  for (const press of presses) {
    ...
    if (press === '@') {
      ...
      await this.focus(key[W3C_WEB_ELEMENT_IDENTIFIER]);
      await B.delay(750);
    }
    await this.click(key[W3C_WEB_ELEMENT_IDENTIFIER]);
  }
...
export async function getAttribute (attribute, elId) {
  const elNode = await this._elementInteractionGuard(elId);
  for (const attr of Object.values(elNode.attributes)) {
