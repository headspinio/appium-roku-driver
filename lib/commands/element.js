...
function elementNodesAreEqual (node1, node2, examineRelatives = true) {
  ...
  if (!_nodeAttrsAreEqual(node1, node2)) {
    ...
    log.debug(JSON.stringify(_nodeAttrsToMap(node1)));
    log.debug(`Node 1 (${node2.nodeName}) attributes:`);
    log.debug(JSON.stringify(_nodeAttrsToMap(node2)));
    return false;
  }
...
function _nodeEqualityAttrsToMap (n) {
  return _nodeEqualityAttrsToMap(n, true);
}

function _nodeAttrsAreEqual (n1, n2) {
  const [n1Attrs, n2Attrs] = [n1, n2].map(_nodeEqualityAttrsToMap);
...
export async function makeNextFocusMove (elNode) {
  ...
  for (const siblingType of [PREV_SIBLING, NEXT_SIBLING]) {
    ...
    while (sibling) {
      log.debug(`Examining sibling of type: ${siblingType}: <${sibling.nodeName}>`);
