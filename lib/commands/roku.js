...
import { VALID_MEDIA_TYPES } from './general';
 ...
 */
export async function roku_ecpInput(params = {}) {
  if (!_.isPlainObject(params)) {
    throw new errors.InvalidArgumentError(`Parameters object for ecpInput must a plain JS object`);
  }
...
/**
 * @this RokuDriver
 * @param {{contentId: string, mediaType?: string}} params
 * @returns {Promise<void>}
 */
export async function roku_deepLink({contentId, mediaType = 'movie'}) {
  if (!VALID_MEDIA_TYPES.has(/** @type {ValidMediaTypes} */(mediaType))) {
    throw new errors.InvalidArgumentError(`mediaType argument '${mediaType}' was not one of the valid media types: ${JSON.stringify(VALID_MEDIA_TYPES)}`);
  }
  await this.roku_ecpInput({contentId, mediaType});
}
...
/**
 * @typedef {import('./general').ValidMediaTypes} ValidMediaTypes
 */
